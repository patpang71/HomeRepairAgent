...
class TestGetUserProfile:
    ...
    def test_returns_profile_by_apple_id(self, mock_conn, mock_cursor):
        ...
        assert result['userName'] == 'patpang'
    ...
    def test_returns_profile_by_google_id(self, mock_conn, mock_cursor):
        google_row = SAMPLE_ROW[:2] + (None, 'google456') + SAMPLE_ROW[4:]
        mock_cursor.fetchall.return_value = [google_row]
    ...
    def test_returns_multiple_projects_for_same_user(self, mock_conn, mock_cursor):
        second_row = SAMPLE_ROW[:10] + ('Second House', 'PLUMBING') + SAMPLE_ROW[12:]
        mock_cursor.fetchall.return_value = [SAMPLE_ROW, second_row]
